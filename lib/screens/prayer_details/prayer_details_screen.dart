...
class _PrayerDetailsState extends State<PrayerDetails> {
  ...
  String reminderString(PrayerDataModel? prayerData) {
    ...
    final notificationText = reminder.frequency == Frequency.weekly
        ? '${reminder.frequency}, ${LocalNotification.daysOfWeek[(reminder.scheduleDate?.weekday ?? 0) > 6 ? 6 : (reminder.scheduleDate?.weekday ?? 0)]}, ${reminder.scheduleDate?.hour == 0 ? 12 : reminder.scheduleDate?.hour}:${(reminder.scheduleDate?.minute ?? 0) < 10 ? '0${reminder.scheduleDate?.minute}' : '${reminder.scheduleDate?.minute}'} ${DateFormat('a').format(reminder.scheduleDate ?? DateTime.now())}'
